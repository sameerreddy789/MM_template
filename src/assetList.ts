const assetList = {
    "register": {
        "images": [
            "./images/registration/bg-extended.webp",
            "./svgs/registration/reg-banner.svg",
            "./svgs/registration/bg-mobile.svg",
            "./svgs/registration/leftarr.svg",
            "./svgs/registration/rightarr.svg",
            "./svgs/registration/dropdown.svg",
            "./svgs/registration/scrollThumb.svg",
            "./svgs/registration/scroll-bar.svg",
            './svgs/registration/back.svg',
            "./images/registration/information.png",
            "./images/registration/info3.png",
        ],
        "videos": []
    },
    "contact": {
        "images": [
            // './images/contact/Aryan.png',
            // './images/contact/Aditya.png',
            // './images/contact/ContactCard2.webp',
            // './images/contact/ContactCard3.webp',
            './images/contact/contact-banner.webp',
            // './images/contact/Ahan.png',
            // './images/contact/Arshita.png',
            // './images/contact/Ayushmaan.png',
            // './images/contact/Dhruv.png',
            // './images/contact/Door1.webp',
            // './images/contact/Door2.webp',
            // './images/contact/Ishita.png',
            // './images/contact/Pranav.png',
            // './images/contact/Rahul.png',
            // './images/contact/SajalY.png',
            './images/contact/ContactCard1.webp',
            './images/contact/DoorsCombined.webp',
            './images/contact/DoorsMobile.webp',
        ],
        "videos": []
    },
    "aboutus": {
        "images": [
            './svgs/aboutus/lines.svg',
            './svgs/aboutus/tree.svg',
            './svgs/aboutus/abt.svg',
            // './svgs/aboutus/dagger.svg',
            // './svgs/aboutus/fan.svg',
            './svgs/aboutus/header.svg',
            // './svgs/aboutus/lantern.svg',
            './svgs/aboutus/letter1.svg',
            './svgs/aboutus/letter2.svg',
            './svgs/aboutus/letter3.svg',
            // './svgs/aboutus/text-frame.svg',
            // './svgs/aboutus/Mask2.svg',
            // './svgs/aboutus/bord.svg',
            './svgs/aboutus/fan.webp',
            './svgs/aboutus/letter4.svg',
            './svgs/aboutus/letter5.svg',
            './svgs/aboutus/letter6.svg',
            './svgs/aboutus/letter7.svg',
            './svgs/aboutus/letter8.svg',
            './svgs/aboutus/next.svg',
            './svgs/aboutus/pause.svg',
            './svgs/aboutus/prev.svg',
            './images/aboutus/background.jpg',
            './images/aboutus/backg.webp',
            './svgs/aboutus/nextarr.svg',
            './svgs/aboutus/play.svg',
            './svgs/aboutus/prvarr.svg',
            './svgs/aboutus/reghead.svg',
            './svgs/aboutus/borde.svg',
            './images/aboutus/abtbck.webp',
        ],
        "videos": []
    },
    "events": {
        "images": [
            './images/events/backg1.png',
            './images/events/Bob.webp',
            './images/events/dancef.webp',
            './images/events/dramaf.webp',
            './images/events/image.png',
            './images/events/misc1.webp',
            './images/events/music.webp',
            './images/events/music1.webp',
            './images/events/musicf.webp',
            './images/events/photography.webp',
            './images/events/proshow.webp',
            './images/events/text.png',
            './images/events/TextMobile.png',
            './images/events/TextMobile1.png',
            './images/events/DramaMobilef.png',
            './images/events/DanceMobilef.png',
            './images/events/MusicMobilef.png',
            './images/events/MiscMobilef.png',
            './images/events/PhotographyMobilef.png',
            './images/events/Eventpics/Agri-Plex.png',
            './images/events/Eventpics/Band Battle.png',
            './images/events/Eventpics/CADathon.png',
            './images/events/Eventpics/Dedicate_a_song.png',
            './images/events/Eventpics/Fashion_show.png',
            './images/events/Eventpics/Folk_dance.png',
            './images/events/Eventpics/Food_challenge.png',
            './images/events/Eventpics/Fusion_dance.png',
            './images/events/Eventpics/Gully_cricket.png',
            './images/events/Eventpics/Hackathon.png',
            './images/events/Eventpics/Jenga.png',
            './images/events/Eventpics/Life_saver_workshop.png',
            './images/events/Eventpics/Push_up_challenge.png',
            './images/events/Eventpics/Robo-Race.png',
            './images/events/Eventpics/Shoota_and_edit.png',
            './images/events/Eventpics/Solo_and_group_singing.png',
            './images/events/Eventpics/Spot_photography.png',
            './images/events/Eventpics/Stunt_show.png',
            './images/events/Eventpics/Talent_show.png',
            './images/events/Eventpics/Talk_shows.png',
            './images/events/Eventpics/Tech-Exhibition.png',
            './images/events/Eventpics/Traditional_Dressing_competition.png',
            './images/events/Eventpics/Treasure_hunt.png',
            './images/events/Eventpics/Tug_of_war.png',
            './images/events/Eventpics/Water-Rocket-Launch.png',
            './images/events/Eventpics/dj_night.png',
            './svgs/events/bottom.svg',
            './svgs/events/cl1.svg',
            './svgs/events/cl2.svg',
            './svgs/events/dan.webp',
            './svgs/events/eventsback.webp',
            './svgs/events/Next1.svg',
            './svgs/events/star.svg',
            './svgs/events/star2.svg',
            './svgs/events/text.svg',
            './svgs/events/top.svg',
            './svgs/events/topright.svg',
            "./svgs/events/location.svg",
        ],
        "videos": []
    },
    "gallery": {
        "images": [
            './images/gallery/close_button.png',
            './images/gallery/photog white logo.png',
        ],
        "videos": []
    }
}

export default assetList;