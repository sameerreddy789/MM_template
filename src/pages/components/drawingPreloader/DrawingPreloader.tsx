...
export default function DrawingPreloader({
  ...
  const desktopSvgWrapperRef = useRef<HTMLDivElement>(null);
  ...
  // Desktop: fetch the sketch and inject it
  useEffect(() => {
    if (isMobile) return;
    let cancelled = false;
    ...
    fetch("/svgs/landing/LatestProcessed.svg")
      .then((res) => res.text())
      .then((markup) => {
        const wrapper = desktopSvgWrapperRef.current;
        ...
        svgEl.current = injected as SVGSVGElement;
  ...
  // Mobile: the sketch is inline, so it only needs measuring
  useEffect(() => {
    if (!isMobile || !svgEl.current) return;
    primePaths(
      Array.from(
        svgEl.current.querySelectorAll<SVGPathElement>("path.drawingPaths")
      )
    );
  }, [isMobile]);
